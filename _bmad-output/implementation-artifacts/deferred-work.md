...
- **D-16-1-1: Per-strategy position/P&L gauges not wired to live strategy state** (`prometheus.py`, `base.py`) — `set_position_size`, `set_unrealized_pnl`, `set_drawdown`, `set_consumer_lag` are implemented but `BaseStrategy` never calls them; Grafana shows zeroes for all strategies. Fix: call `set_position_size` etc. from each strategy's signal handler when position changes are detected, or add a position-tracking mixin to `BaseStrategy`.
...
- **D-16-2-2: QuestDB healthcheck uses `/health` endpoint — verify older QuestDB version support** (`docker-compose.yml`) — QuestDB < 7.0 does not expose `/health`; the existing healthcheck using `/exec?query=select+1` was more broadly compatible. Confirm current deployment QuestDB version supports `/health` before relying on `depends_on: condition: service_healthy`.
